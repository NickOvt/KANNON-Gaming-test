...
function CountryList() {
  ...
  function getCountriesList(inputValue) {
    ...
    const clearedCountriesInputString = inputValue.replace(
      countryListRegex,
      ''
    );
    // Make an array of the entered list and remove whitespaces
    const countriesArray = clearedCountriesInputString
      .split(',')
      .map((el) => el.trim());
    ...
    const passValidation = initialCountriesInput.every((el) =>
      singleCountryNameRegex.test(el)
    );
